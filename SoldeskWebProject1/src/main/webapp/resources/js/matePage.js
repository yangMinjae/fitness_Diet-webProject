...
function updatePagination(totalPages, currentPage) {
    ...
    // 이전 버튼
    if (currentPage > 1) {
        paginationHTML += `<a href="javascript:void(0);" class="page-btn" onclick="loadPage(${currentPage - 1})">이전</a>`;
    }

    // 페이지 번호들
    for (var i = 1; i <= totalPages; i++) {
        paginationHTML += `<a href="javascript:void(0);" class="page-btn ${i == currentPage ? 'active' : ''}" onclick="loadPage(${i})">${i}</a>`;
    }

    // 다음 버튼
    if (currentPage < totalPages) {
        paginationHTML += `<a href="javascript:void(0);" class="page-btn" onclick="loadPage(${currentPage + 1})">다음</a>`;
    }
